...
static int my_unlink(const char * path) {
	...
	if((idxDir = findFileByName(&myFileSystem, (char *)path + 1)) != -1) {
        ...
        if (resizeNode(idxNodoI, 0) < 0) return -EIO;
	    myFileSystem.directory.files[idxDir].freeFile = true;
	    myFileSystem.directory.numFiles--;
	    myFileSystem.nodes[idxNodoI]->freeNode = true;
	    myFileSystem.numFreeNodes++;
	    if(updateDirectory(&myFileSystem) == -1) return -1;
	    if(updateNode(&myFileSystem, idxNodoI, myFileSystem.nodes[idxNodoI]) == -1) return -1;
...
static int my_read(const char * filename, char * c , size_t size , off_t off, struct fuse_file_info * fuse_info){
	char buffer[BLOCK_SIZE_BYTES];
    int bytes2Read = size, totalRead = 0;
    NodeStruct *node = myFileSystem.nodes[fuse_info->fh];
    fprintf(stderr, "--->>>my_read: filename %s, size %zu, offset %jd, fh %"PRIu64"\n", filename, size, (intmax_t)off, fuse_info->fh);

    // Read data
   while(bytes2Read) {
        int i;
        int currentBlock, offBlock;
        currentBlock = node->blocks[off / BLOCK_SIZE_BYTES];
        offBlock = off % BLOCK_SIZE_BYTES;
...
struct fuse_operations myFS_operations = {
    ...
    .unlink		= my_unlink,					// Borra un fichero
    .read 		= my_read,						// Read a opened file
};
