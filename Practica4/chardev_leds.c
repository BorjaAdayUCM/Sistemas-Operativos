...
static ssize_t device_write(struct file *filp, const char *buff, size_t len, loff_t * off) {
    unsigned int state = ALL_LEDS_OFF;
    int bytes_to_write = len;
    char buffWrite[BUF_LEN];
    int i = 0;

    if(bytes_to_write > BUF_LEN)
        return -ENOSPC;

    if(copy_from_user(buffWrite, buff, bytes_to_write))
        return -EFAULT;
    
   for(i = 0; i < bytes_to_write - 1; i++){
        if(buffWrite[i] == '1') state |= 0x2;
        else if (buffWrite[i] == '2') state |= 0x4;
        else if (buffWrite[i] == '3') state |= 0x1;
        else {
            set_leds(kbd_driver, ALL_LEDS_OFF);
            return len;
        }
